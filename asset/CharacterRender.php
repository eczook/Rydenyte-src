...
$appearanceParts = [];
$appearanceParts[] = "http://www.ryblox.xyz/asset/BodyColors.ashx?userId=$userId";
...
foreach ($wearingItems as $item) {
    if (!empty($item["asset_id"])) {
        if ($item["category"] !== 17 && $item["category"] !== 18 && $item["category"] !== 19) {
            $appearanceParts[] = "http://www.ryblox.xyz/asset/?id=" . $item["asset_id"];
        }
    }
}
